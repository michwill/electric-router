...
# initcode) alone, so one recipe puts the quoter at the *same* address everywhere
# and one whitelist entry covers the lot.
...
def deploy_one(name: str, args, account=None) -> int:
    ...
    if args.broadcast:
        ...
        # fork still holds the contract it just rehearsed deploying; without it
        # every chain after the first fails with "Cannot fork with dirty state".
        # Nothing is carried over -- each fork starts from that chain's head.
        # Pinned to a real block rather than boa's default `safe` tag, which
        # polygon's endpoint does not serve and which would drop that chain.
        from erouter.dev.rpc import JsonRpcTransport
